...
<div class="span-24" id="portfolio">
    <h1>{$lang.movietag_title|replace:'#tag#':$tag}</h1>
    
    <div class="left">{$lang.sorting}: </div>
    <div class="left filter-title myfilter">
        <ul id="filter">
            {if $global_settings.seo_links}
                <li{if $sortby eq 'abc'} class="current"{/if}><a href="{$baseurl}/{$routes.movie_tag}/{$tag_perma}/abc">{$lang.sorting_abc}</a></li>
                <li{if $sortby eq 'date'} class="current"{/if}><a href="{$baseurl}/{$routes.movie_tag}/{$tag_perma}/date">{$lang.sorting_newest}</a></li>
                <li{if $sortby eq 'imdb_rating'} class="current"{/if}><a href="{$baseurl}/{$routes.movie_tag}/{$tag_perma}/imdb_rating">{$lang.sorting_imdb}</a></li>
            {else}
                <li{if $sortby eq 'abc'} class="current"{/if}><a href="{$baseurl}/index.php?menu=movie-tag&tag={$tag_perma}&sortby=abc">{$lang.sorting_abc}</a></li>
                <li{if $sortby eq 'date'} class="current"{/if}><a href="{$baseurl}/index.php?menu=movie-tag&tag={$tag_perma}&sortby=date">{$lang.sorting_newest}</a></li>
                <li{if $sortby eq 'imdb_rating'} class="current"{/if}><a href="{$baseurl}/index.php?menu=movie-tag&tag={$tag_perma}&sortby=imdb_rating">{$lang.sorting_imdb}</a></li>            
            {/if}
        </ul>
    </div>
    ...
    {if $tag neq ''}
        ...
        {else}
            <div class="span-24">
                <center><br /><br />{$lang.movietag_no_movie}<br /><br /></center>
            </div>
        {/if}
    {else}
        <div class="span-24">
            <center><br /><br />{$lang.movietag_no_category}<br /><br /></center>
        </div>
    {/if}
</div>
